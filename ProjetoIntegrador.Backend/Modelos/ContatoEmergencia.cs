...
public class ContatoEmergencia
{
    ...
    {
        if (string.IsNullOrWhiteSpace(nome)) throw new Exception("Nome do usuário não pode estar vazio!");
        if (nome.Length < 3) throw new Exception("Nome do usuário precisa de no mínimo 3 caracteres!");
    ...
    {
        if (string.IsNullOrWhiteSpace(email)) throw new Exception("Email não pode estar vazio!");
        if (email.Length > 100) throw new Exception("Email pode ter no máximo 100 caracteres!");
